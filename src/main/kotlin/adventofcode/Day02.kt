...
fun main(args: Array<String>) {
    ...
    val input = "3093\t749\t3469\t142\t2049\t3537\t1596\t3035\t2424\t3982\t3290\t125\t249\t131\t118\t3138\n" +
    ...
    println(Day02().calcChecksum(input))
    println(Day02().calcChecksumPart2(input))
...
class Day02 {
    ...
    fun calcChecksum(input: String): Int {
        return input
                ...
                .sumBy { rowMax(it) - rowMin(it) }
    }

    fun calcChecksumPart2(input: String): Int {
        return input
                .split("\n")
                .map { it.split("\t").map { it.toInt() } }
                .map( { isEvenlyDevisable(it) })
                .sumBy { it.first / it.second }
    }

    private fun isEvenlyDevisable(values: List<Int>): Pair<Int, Int> {

        return values
                .sorted()
                .flatMap { x -> values.map { Pair(x, it) } }
    ...
    private fun rowMin(input: List<Int>): Int {
        return input.min() ?: throw IllegalArgumentException("null?")
    }

    private fun rowMax(input: List<Int>): Int {
        return input.max() ?: throw IllegalArgumentException("null?")
    }
